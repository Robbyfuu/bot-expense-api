# PROYECTO: BOT DE GASTOS WHATSAPP (MIGRADO A OPENAI)

> ⚠️ **IMPORTANTE**: Este proyecto ha migrado de Google Gemini a **OpenAI GPT-4o** debido a cuotas y rendimiento.

Por favor, revisa el archivo [README.md](./README.md) para la documentación actualizada, stack tecnológico y guía de uso.

## Historial de Cambios (Gemini -> OpenAI)

- Se reemplazó `@google/generative-ai` por `openai`.
- Se implementó `Merchant` table para normalización de comercios (RUT).
- Validación de gastos con estado `PENDING` -> `CONFIRMED`.
